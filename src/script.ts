...
document.addEventListener('DOMContentLoaded', () => {
  ...
  class FluidSimulation {
    ...
    private waterColor: string = '#ffffff';
    private waterColorHL: string = 'rgba(255,255,255,0.3)';
    private bgColor: string = '#1a1a1a';
    private w: number = 0;
    private h: number = 0;
    ...
    private _makeHighlight(color: string): string {
    ...
    private _isInsideShape(nx: number, ny: number): boolean {
      if (this.isCircle) {
        const dx = nx - 0.5;
        const dy = ny - 0.5;
    ...
    private _getNormalized(e: MouseEvent): { nx: number, ny: number } {
      const rect = this.el.getBoundingClientRect();
      return {
  ...
  const lifeExpectancyInput = document.getElementById('life-expectancy-input') as HTMLInputElement;
  const sizeInput = document.getElementById('size-input') as HTMLInputElement;
  const fontSizeInput = document.getElementById('font-size-input') as HTMLInputElement;

  // Custom Controls (Toggles)
  const transparentTimerToggle = document.getElementById('transparent-timer') as HTMLButtonElement;
  const boldTimerToggle = document.getElementById('bold-timer') as HTMLButtonElement;
  const showAgeToggle = document.getElementById('show-age') as HTMLButtonElement;

  // Custom Controls (Segmented)
  const precisionGroup = document.getElementById('precision-group') as HTMLDivElement;
  const shapeGroup = document.getElementById('shape-group') as HTMLDivElement;
  const unitPrecisionGroup = document.getElementById('unit-precision-group') as HTMLDivElement;
  const typographyGroup = document.getElementById('typography-group') as HTMLDivElement;
  ...
  async function loadSettings() {
    try {
      const api = typeof chrome !== 'undefined' && chrome.storage ? chrome : (window as any).browser;
      if (api && api.storage) {
        const result = await new Promise<{ settings?: Partial<Settings> }>((resolve) => {
          api.storage.local.get('settings', (res: any) => resolve(res));
        });
        if (result.settings) {
  ...
  function applyAppearance() {
    ...
    if (fluidSim) {
      ...
      requestAnimationFrame(() => {
        if (fluidSim) fluidSim.readColors();
      });
    }
  }
  ...
  function syncStateToUI() {
    ...
    if (dobYear) dobYear.value = y;
    if (dobMonth) dobMonth.value = m;
    if (dobDay) dobDay.value = d;

    if (lifeExpectancyInput) lifeExpectancyInput.value = settings.lifeExpectancy.toString();
    if (sizeInput) sizeInput.value = settings.size.toString();
    if (fontSizeInput) fontSizeInput.value = (settings.timerFontSize || 6).toString();

    // Toggles
    if (transparentTimerToggle) transparentTimerToggle.setAttribute('aria-checked', settings.transparentTimer.toString());
    if (boldTimerToggle) boldTimerToggle.setAttribute('aria-checked', settings.boldTimer.toString());
    if (showAgeToggle) showAgeToggle.setAttribute('aria-checked', settings.showAge.toString());

    // Segmented Buttons
    if (precisionGroup) {
      Array.from(precisionGroup.children).forEach((btn) => {
        if (btn instanceof HTMLButtonElement) {
          if (btn.getAttribute('data-value') === settings.subDayPrecision) btn.classList.add('active');
    ...
    if (shapeGroup) {
      Array.from(shapeGroup.children).forEach((btn) => {
        if (btn instanceof HTMLButtonElement) {
          if (btn.getAttribute('data-value') === settings.shape) btn.classList.add('active');
          else btn.classList.remove('active');
        }
      });
    }
    ...
    if (typographyGroup) {
      Array.from(typographyGroup.children).forEach((btn) => {
        if (btn instanceof HTMLButtonElement) {
          if (btn.getAttribute('data-value') === (settings.typography || 'inter')) btn.classList.add('active');
          else btn.classList.remove('active');
        }
      });
    }
    ...
    if (unitPrecisionGroup) {
      Array.from(unitPrecisionGroup.children).forEach((btn) => {
        if (btn instanceof HTMLButtonElement) {
          if (btn.getAttribute('data-value') === settings.unitPrecision) btn.classList.add('active');
          else btn.classList.remove('active');
        }
      });
    }

    const themeGroup = document.getElementById('theme-group') as HTMLDivElement;
    if (themeGroup) {
  ...
  function setupEventListeners() {
    ...
    if (dobYear) dobYear.addEventListener('input', (e) => {
      dobHandler();
      if (dobYear.value.length === 4 && dobMonth) dobMonth.focus();
    });
    if (dobMonth) dobMonth.addEventListener('input', (e) => {
      dobHandler();
      if (dobMonth.value.length === 2 && dobDay) dobDay.focus();
    });
    if (dobDay) dobDay.addEventListener('input', (e) => {
      dobHandler();
      if (dobDay.value.length === 2) dobDay.blur();
    });

    if (lifeExpectancyInput) lifeExpectancyInput.addEventListener('change', (e) => {
      settings.lifeExpectancy = parseInt((e.target as HTMLInputElement).value, 10) || 80;
      saveSettings();
    });

    if (sizeInput) sizeInput.addEventListener('input', (e) => {
      settings.size = parseInt((e.target as HTMLInputElement).value, 10);
      saveSettings();
    });

    if (fontSizeInput) fontSizeInput.addEventListener('input', (e) => {
      settings.timerFontSize = parseFloat((e.target as HTMLInputElement).value);
      saveSettings();
    });

    // Toggle Handlers
    const toggles = [
      { el: transparentTimerToggle, key: 'transparentTimer' as keyof Settings },
      { el: boldTimerToggle, key: 'boldTimer' as keyof Settings },
      { el: showAgeToggle, key: 'showAge' as keyof Settings },
    ];
    ...
    // Segmented Handlers
    const setupSegmented = (group: HTMLDivElement | null, key: 'shape' | 'unitPrecision' | 'subDayPrecision' | 'typography') => {
      if (!group) return;
      Array.from(group.children).forEach((btn) => {
        if (btn instanceof HTMLButtonElement) {
          btn.addEventListener('click', () => {
            ...
            settings[key] = newValue as never;
  ...
  // --- Grid Mathematics ---
  function renderGrid() {
    if (!gridContainer || window.innerWidth < 10 || window.innerHeight < 10) return;
    ...
    if (totalUnits > 40 && bestCols < 2) bestCols = Math.ceil(Math.sqrt(totalUnits));
    if (!isFinite(bestCols) || bestCols < 1) bestCols = 1;
    ...
    if (settings.unitPrecision === 'weeks') {
      ...
      if (now.getDate() < compareDay) unitsLived--;
    } else {
    ...
    for (let i = 0; i < totalUnits; i++) {
      ...
      if (i === unitsLived) {
        ...
        requestAnimationFrame(() => {
          if (fluidSim) { fluidSim.destroy(); fluidSim = null; }
  ...
  function updateTimerLoop(timestamp: number) {
    updateTimerLogic();
  ...
  function updateTimerLogic() {
    if (targetCache.dob !== settings.dob || targetCache.lifeExpectancy !== settings.lifeExpectancy) {
      ...
      if (isNaN(parsedDob.getTime())) return;
...
(function () {
    const faviconBase64 = 'data:image/png;base64,';
